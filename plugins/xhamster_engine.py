...
def _candidate_video_urls_from_initials(initials, current_url: str):
    ...
    def add(u):
        ...
        # string ke andar multiple urls ho sakte hain
        for m in re.finditer(r"https?://[^\s<>\"']+", u):
            cand = m.group(0).strip().strip("`'\"<>[]()")
            cand = cand.replace("&amp;", "&")
            if "/videos/" not in cand and "/movies/" not in cand:
                continue
            if not is_xhamster(cand):
                continue
    ...
    if isinstance(initials, dict):
        # urls key ko sabse pehle priority
        urls_node = initials.get("urls")
        for val in _walk_strings(urls_node):
            add(val)
        # pageHiddenLink/canonical/fallback jaisi fields
    ...
    # current URL ko end me rakho (agar duplicate nahi)
